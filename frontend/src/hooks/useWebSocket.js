...
export function useWebSocket(path = '', autoConnect = false) {
  ...
  const scheduleRetry = useCallback(() => {
    if (retryCount.current >= MAX_RETRIES) {
      ...
      toast.error('WebSocket: max reconnect attempts reached')
    ...
    retryTimer.current = setTimeout(() => {
      if (shouldRetry.current) connectInternal()
    }, delay)
  }, [setWsStatus, setWsError])

  const connectInternal = useCallback(() => {
    if (!wsRef.current) wsRef.current = createWebSocketManager(path)
    ...
    ws.on('open', () => {
      ...
      toast.success('WebSocket connected', { id: 'ws-status' })
    })
    ...
    ws.on('close', (event) => {
      ...
      if (shouldRetry.current && event.code !== 1000) scheduleRetry()
    })
